...
- **自分から外部へ問い合わせに行くのは更新の確認だけ** — 使用状況の送信も、テーマのダウンロードも、画像アップロード連携もありません。書いた内容が外部へ送信されることはありません（文書中の`https://`の画像や埋め込みは既定では読み込まず、プレースホルダを表示します。読み込むと参照先に「開かれたこと」が伝わるためです。詳細は取扱説明書「外部通信について」）
- **Markdown を書いたまま整形して見せる** — 見出し・表・数式・Mermaid図・脚注・Callouts に対応
- **約60種類の言語のシンタックスハイライト** — 折りたたみ、インデントガイド、カラープレビュー付き
- **文字コードと改行コードを保って開き直す** — Shift_JIS の古いテキストもそのまま扱えます
- **9種類のテーマ** — カスタム CSS も読み込めます。見本を見ながら色や書体を選んで作る画面もあります
...
新しい版があれば、そのままダウンロードして入れ替えられます（ダウンロードしたZipのSHA256を照合します。ただしこのハッシュ値もZip本体と同じGitHubから取得しているため、転送中の破損は検出できますが、配布元やGitHubアカウントが乗っ取られて中身が差し替えられた場合までは防げません）。
