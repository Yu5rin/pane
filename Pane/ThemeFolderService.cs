...
internal static class ThemeFolderService
{
    ...
    private const string SampleCssContent = """
        /* ==============================================================
           ...
           外部通信について: このファイルにWebフォントの @import・外部画像のURL・
           フォントCDN等を書いても読み込まれません(通信そのものが行われません)。
           フォントはOSにインストール済みのフォント名を指定する形だけが
           サポートされます。Pane本体が外部と通信するのは、新しい版があるかを
           尋ねるときだけです(設定「バージョン情報」)。
           ============================================================== */
        ...
        */
        ...
        /* ---- インデントガイド(ネストの深さを示す縦線) ----
           この線の色は上の変数一覧の --rule(通常時)・--fold-guide-hover(折りたたみ
           マーカーにマウスを乗せている間だけの強調色)の2つの変数だけで調整できます。
           セレクタを直接書く必要はないので、下の例は基本的に使わないことをおすすめします。 */

        /* 補足: どうしてもセレクタを直接書きたい場合の注意
           この線はCodeMirrorのEditorView.theme()という仕組みでJS側から動的にスタイルが
           注入されており、詳細度(CSSの優先順位)がやや高めです。単に`.cm-guide-line { ... }`
           とだけ書いても反映されないため、実在するid(#cm-host)を頭に付けて詳細度を
           上げる必要があります。ただし、通常時の行(.cm-guide-line)だけを#cm-host付きで
           上書きすると、今度は逆に#cm-host付きセレクタの詳細度が、マーカーホバー時だけ
           効く強調用の指定(.cm-guide-line.cm-guide-hot)より高くなってしまい、マウスを
           乗せても線の色がホバー用の色に切り替わらなくなります(id付きセレクタはclassが
           何個並んだ指定よりも常に優先されるため)。セレクタを直接書く場合は、下のように
           通常時・ホバー時の両方を同じく#cm-host付きで、必ずセットで上書きしてください。 */
        /*
        #cm-host .cm-guide-line { background-color: var(--accent); }
        #cm-host .cm-guide-line.cm-guide-hot { background-color: var(--accent-hover); }
        */
        /* インデントガイドの縦線が通常時はアクセントカラーに、折りたたみマーカーに
           マウスを乗せている間だけ少し濃い--accent-hoverに変わります。上の2行は必ず
           セットで書いてください。1行目だけ書くと、ホバー用の指定より詳細度が高く
           なってしまい、マウスを乗せても色が変わらなくなります。 */
