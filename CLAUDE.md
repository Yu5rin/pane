...
- 点検で見つかった問題と、その修正の経緯は `docs/調査記録/` に残す。
  **なぜそう直したか、何を調べて何が違ったか、何を直さなかったか**の記録で、
  ソースコードのコメントからも根拠として参照している。
  `docs/調査記録/README.md` に索引と、繰り返し出てきた誤りの一覧がある。
  配色や実測を扱う前に、その「繰り返し出てきた誤り」の節を読むこと。
...
4. **リリース本文の案をこちらで用意して添える。** 利用者向けの言葉で書き、
   ...
   （本文に何を書き、何を書かないかは次の「リリース本文とREADMEの分担」に従う）
5. 公開はユーザーが行う。こちらから公開しない

## リリース本文とREADMEの分担

一般的なリポジトリと同じく、**アプリの説明はトップページ（README）、リリースにはその版の
変更点だけ**を置く。

- **README に置くもの**: アプリの説明・画面写真・主な特徴・動作環境・インストール・
  更新方法・使い方の入口・ビルド。版をまたいで変わらない説明はすべてこちら
...
以前はリリースごとに使い方や更新方法を書き足していたため、同じ説明が版の数だけ並び、
変更点が埋もれていた（2026-09-25、ユーザーの所有する各リポジトリをこの形に揃えることにした）。
