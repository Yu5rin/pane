...
function inlineHtml(doc, tree, from, to) {
  let html = "";
  let pos = from;
  ...
  function render(n) {
    ...
    while (c) {
      ...
      if (c.from > pos) { html += inlineTextToHtml(doc.sliceString(pos, c.from)); pos = c.from; }
      switch (c.name) {
        case "StrongEmphasis": html += `<strong>${inlineHtml(doc, tree, c.from + 2, c.to - 2)}</strong>`; pos = c.to; break;
        case "Emphasis": html += `<em>${inlineHtml(doc, tree, c.from + 1, c.to - 1)}</em>`; pos = c.to; break;
        case "Strikethrough": html += `<del>${inlineHtml(doc, tree, c.from + 2, c.to - 2)}</del>`; pos = c.to; break;
        case "Superscript": html += `<sup>${inlineHtml(doc, tree, c.from + 1, c.to - 1)}</sup>`; pos = c.to; break;
        case "Subscript": html += `<sub>${inlineHtml(doc, tree, c.from + 1, c.to - 1)}</sub>`; pos = c.to; break;
        ...
        case "Link": {
          ...
          if (/^\[\^[^\]]+\]$/.test(text)) {
            ...
            html += href
              ? `<a href="${escText(href)}">${inlineHtml(doc, tree, textFrom, textTo)}</a>`
              : inlineTextToHtml(text);
          }
          pos = c.to;
          break;
        }
  ...
  render(container);
  if (pos < to) html += inlineTextToHtml(doc.sliceString(pos, to));
  return html;
}

function renderList(doc, tree, node, ordered) {
  ...
  for (let item = node.firstChild; item; item = item.nextSibling) {
    ...
    for (let c = item.firstChild; c; c = c.nextSibling) {
      if (c.name === "Paragraph") {
        inner += inlineHtml(doc, tree, c.from, c.to);
      } else if (c.name === "Task") {
        ...
        inner += inlineHtml(doc, tree, textFrom, c.to);
      } else if (c.name === "BulletList") {
        nestedList += renderList(doc, tree, c, false);
      } else if (c.name === "OrderedList") {
        nestedList += renderList(doc, tree, c, true);
      }
    }
...
function renderTable(doc, tree, node) {
  ...
  for (const row of node.getChildren("TableRow")) {
    ...
    for (const cell of row.getChildren("TableCell")) {
      html += `<td>${inlineHtml(doc, tree, cell.from, cell.to)}</td>`;
    }
    html += "</tr>";
  }
  html += "</tbody></table>";
...
function renderBlockquote(doc, tree, node) {
  let html = "<blockquote>";
  for (let c = node.firstChild; c; c = c.nextSibling) {
    html += renderBlock(doc, tree, c);
  }
  html += "</blockquote>";
  return html;
}
...
function renderBlock(doc, tree, node) {
  switch (node.name) {
    ...
    case "ATXHeading4": case "ATXHeading5": case "ATXHeading6": {
      ...
      return `<h${level}>${inlineHtml(doc, tree, from, node.to)}</h${level}>`;
    }
    case "SetextHeading1": return `<h1>${inlineHtml(doc, tree, node.from, node.to)}</h1>`;
    case "SetextHeading2": return `<h2>${inlineHtml(doc, tree, node.from, node.to)}</h2>`;
    case "Paragraph": return `<p>${inlineHtml(doc, tree, node.from, node.to)}</p>`;
    case "BulletList": return renderList(doc, tree, node, false);
    case "OrderedList": return renderList(doc, tree, node, true);
    case "Blockquote": return renderBlockquote(doc, tree, node);
    case "FencedCode": return renderFencedCode(doc, node);
    case "Table": return renderTable(doc, tree, node);
...
export function renderMarkdownToHtml(state, { from = 0, to = state.doc.length } = {}) {
  const doc = state.doc;
  const tree = syntaxTree(state);
  let html = "";
  for (let node = tree.topNode.firstChild; node; node = node.nextSibling) {
    if (node.to <= from || node.from >= to) continue;
    html += renderBlock(doc, tree, node);
  }
...
// スタイルなし版(X-03)になる。
export function renderStandaloneHtml(state, title, css, styled) {
  const body = renderMarkdownToHtml(state);
