...
export const TOOLTIP_CATEGORIES = {
  ...
  banner: "通知バナー",
  settings: "設定画面",
  // 依頼①(コードコピーボタン)で追加。本文中(CodeMirrorのwidget decoration)に動的生成される
  // 要素向けのカテゴリ。既存のカテゴリはすべて画面の固定UI(メニューバー等)向けだったため新設した。
  editor: "本文中の要素",
};
...
export const TOOLTIPS = {
  ...
  // 「その行の先頭(開始行)から」に改めた。
  // 文言更新(Windows実機フィードバックを受けた「L字」廃止に伴う): 折りたたみ範囲の
  // 最終行で縦線の右に短い横棒を出す「L字」終端表現は廃止済み(現在は縦線がまっすぐ
  // 伸びるだけ)。説明文からもL字への言及を削除した(GuideLineWidget/.cm-guide-line、
  // src/editor.js参照)。
  codeIndentGuides: {
    category: "settings",
    minimalMode: "suppress", // 設定画面の項目はラベル自体で何の設定か分かるため(他の設定項目と同じ扱い)
    standard: "コードモードで、インデントの深さを示す縦線をどこまで表示するかです",
    detailed: "コードモードのインデントガイド(縦線)の表示範囲を選びます。「表示しない」は縦線を一切引きません。「折りたたみできる範囲のみ」(既定)は、関数・オブジェクト・配列など折りたたみマーカーが実際にある階層にだけ縦線を引きます。「すべてのインデント」は、折りたたみとは無関係にインデントの深さすべてに一律で縦線を引きます。いずれも線はまっすぐ引くだけで、終端に横棒は出ません。",
  },

  // ---- 本文中の要素(依頼①: コードブロックのコピーボタン) ----
  // 本文の装飾としてCodeMirrorのwidget decorationで動的生成されるボタン(CodeCopyWidget)。
  // 固定のHTML要素ではないため、生成時(toDOM())にeditor.js側からapplyTooltip()を明示的に
  // 呼んでtitleを設定している(id属性は付けず、data-tip="cm-code-copy"で引く。コードブロックが
  // 1文書に複数あってもidが重複しないようにするため)。
  "cm-code-copy": {
    category: "editor",
    label: "コードをコピー",
    minimalMode: "label",
    standard: "このコードブロックの中身をクリップボードへコピーします",
    detailed: "コードフェンス(```)で囲まれたこのコードブロックの中身だけをクリップボードへコピーします。行番号を表示している場合でも、コピーした内容に行番号自体は含まれません。コピーに成功すると、数秒だけボタンの見た目がチェックマークに変わり「コピーしました」と表示して知らせます。ボタンはカーソルがこのコードブロックの中にあるとき、またはマウスを乗せたときだけ表示されます。",
  },
  ...
  "btn-menu-help": {
    ...
    label: "取扱説明書",
    minimalMode: "label",
    // 文言修正(総点検M13): 実際の動作は「ヘルプメニューを開く」ではなく、取扱説明書
    // ウィンドウ(Pane/HelpWindow.cs、src/help-entry.js)をそのまま開く(help.manualと同じ動作。
    // src/commands.jsのbtn-menu-help配線参照)。以前の文言は存在しない「ヘルプメニュー」を
    // 指しており実態と食い違っていた。バージョン情報は設定画面側にあるため、ここでは触れない。
    standard: "取扱説明書を開きます",
    detailed: "困ったときの手がかりをまとめた取扱説明書を開きます(F1キーでも開けます)。バージョン情報は「設定」の「バージョン情報」で確認できます。",
  },
  ...
  "tabbar-new": {
    ...
    detailed: "現在のウィンドウの中に新しいタブを追加し、無題の文書を新規作成します。ウィンドウ自体を増やす「新しいウィンドウ」とは別の操作です。",
  },
  ...
  "sidebar-tab-files": {
    ...
    label: "ファイルリスト",
    minimalMode: "label",
    standard: "読み込んでいるフォルダ内のファイルを一覧表示します",
    // 「最近使ったファイル」(ファイルメニュー内のサブメニュー)とは別物。こちらは今読み込んでいる
    // フォルダの中身の一覧で、開いたことが無いファイルも表示される(sidebar.js renderFiles参照)。
    detailed: "現在読み込んでいるフォルダの中にあるファイルを、サブフォルダの中身もまとめて平坦な一覧で表示します。クリックするとそのファイルを開きます。フォルダをまだ読み込んでいない場合は「フォルダを開く」から選べます(「最近使ったファイル」とは別の一覧です)。",
  ...
  "status-zoom": {
    ...
    detailed: "既定の文字サイズに対する、現在の本文の文字サイズの拡大率を表示します。Ctrl+マウスホイールで変更でき(「設定」の「一般」でオン/オフ可)、クリックすると100%(既定サイズ)に一発で戻せます。",
  },
  ...
  pregenerateWindows: {
    category: "settings",
    minimalMode: "suppress",
    standard: "設定と取扱説明書の画面を、開く前に用意しておくかどうかです",
    detailed: "オンにすると、本体のウィンドウが落ち着いたあとで設定画面と取扱説明書の画面を先に作っておき、実際に開いたときは35ms前後で表示されます。オフにすると開いたその場で作るため134ms前後になりますが(いずれも実測)、まだ開いていない画面ぶんのメモリ(実測で約160MB)を使わずに済みます。",
  },
  autoSaveEnabled: {
    category: "settings",
    minimalMode: "suppress",
    standard: "編集中の内容を、一定間隔でバックアップ(スナップショット)として保存しておくかどうかです。元のファイルは書き換えません",
    detailed: "オンにすると、下の「自動保存の間隔」で指定した秒数ごとに、編集中の内容を%LOCALAPPDATA%\\Pane\\autosave\\ へバックアップ(スナップショット)として書き出します。元のファイルはこれによって書き換わりません。Paneが正常終了できなかった場合、次回起動時にこのバックアップから復元するか確認されます。ファイル自体を保存するには、これとは別にCtrl+Sでの手動保存が必要です。",
  },
  defaultEncoding: {
    category: "settings",
    minimalMode: "suppress",
  ...
  calloutsEnabled: {
    category: "settings",
    minimalMode: "suppress",
    standard: "> [!NOTE]のようなCallouts(注釈枠)の記法を使えるようにするかどうかです",
    detailed: "オンにすると、引用の先頭に[!NOTE]や[!WARNING]などを書くことで、種類ごとに色分けされたCallouts(注釈枠)として表示します。オフにすると通常の引用として表示されます。",
  },
  ...
  useSeparateThemeInDarkMode: {
    category: "settings",
    minimalMode: "suppress",
    standard: "ダークモードのときだけ、ライトモードとは別の配色プリセットを使うかどうかです",
    detailed: "オンにすると、ライトモード用・ダークモード用でそれぞれ別の配色プリセット(下の「ライトテーマ」「ダークテーマ」)を使い分けます。オフにすると、ダークモードでも「ライトテーマ」で選んだプリセットをそのまま使います(配色自体はダーク基調のまま、装飾のプリセットだけライト用を流用する形です)。",
  },
  ...
  refreshFileAssociation: {
    category: "settings",
    label: "関連付けを今のPaneに更新",
    minimalMode: "suppress",
    standard: "チェックしている拡張子の関連付けを、いま動いているPaneへまとめて登録し直します",
    detailed: "Paneはインストーラを使わない配布形式のため、実行ファイル(Pane.exe)をどこに置いても動きます。その代わり、ファイルの関連付けには登録した時点の実行ファイルの場所がそのまま記録されるので、Paneを別のフォルダへ移したり、新しいバージョンへ入れ替えたりすると、ファイルをダブルクリックしたときには古い方のPaneが起動し続けます(修正したはずの不具合が直っていないように見える、という食い違いの原因になります)。このボタンを押すと、いまチェックが入っている拡張子すべての関連付けを、いま動いているこのPaneへまとめて登録し直します。押したときだけ書き換わり、設定画面を開いただけでは何も変わりません。",
  },
  addToPath: {
    category: "settings",
    minimalMode: "suppress",
