...
export function detectContentMode(text) {
  ...
  // 2. 確実な手がかり(シバン行・宣言等)。Markdownと断定できるほどではなかった場合に、
  //    コードだと確信できる強いシグナルがあればそちらを優先する。
  for (const sig of STRONG_SIGNALS) {
    if (sig.re.test(raw)) {
      const language = sig.language === "c" && CPP_HINT_RE.test(raw) ? "cpp" : sig.language;
      return { mode: "code", language, confidence: 0.92, reason: sig.reason };
    }
  }

  // 3. JSON: JSON.parseが成功し、かつ結果がオブジェクト/配列の場合のみ(数値・文字列だけの
  //    短いテキストがJSONと誤判定されるのを防ぐ)。
  if (/^[[{]/.test(trimmed)) {
    try {
      const parsed = JSON.parse(trimmed);
      if (parsed !== null && typeof parsed === "object") {
        return { mode: "code", language: "json", confidence: 0.95, reason: "JSONとして解析可能" };
      }
    } catch { /* JSONではない。次の判定へ */ }
  }

  // 4. 予約語・記号のスコアリング。最高得点の言語が閾値を超えていればそれを採用する。
  let best = null;
